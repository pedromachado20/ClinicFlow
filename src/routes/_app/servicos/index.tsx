...
function ServicosPage() {
  ...
  function abrirNovo() {
    setEditando(null);
    setCatSel("");
    reset({ nome: "", categoria: "", preco: "", duracao: 30, descricao: "" });
    setOpen(true);
  }
  ...
  const excluir = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["servicos"] }); toast.success("Serviço removido"); setExcluindo(null); },
    onError: () => toast.error("Erro ao remover"),
  });

  function handlePrint() {
    printTable("Serviços", ["Nome", "Categoria", "Preço", "Duração (min)"],
    ...
    <div className="space-y-4">
        ...
        <DialogContent>
          ...
          <form onSubmit={handleSubmit((v) => salvar.mutate(v))} className="space-y-3">
            <div className="space-y-1.5">
              <Label>Nome *</Label>
              <Input {...register("nome")} placeholder="Ex: Consulta Clínica" />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label>Preço (R$)</Label>
                <Input {...register("preco")} placeholder="200.00" />
              </div>
              <div className="space-y-1.5">
                <Label>Duração (min)</Label>
                <Input type="number" {...register("duracao")} placeholder="30" />
              </div>
            </div>
            <div className="space-y-1.5">
      ...
      <Dialog open={!!excluindo} onOpenChange={(o) => !o && setExcluindo(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">Agendamentos existentes não serão afetados.</p>
