...
const assinarAgora = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!tenant.cnpj) throw new Error("Preencha o CPF ou CNPJ da clínica antes de assinar");
        ...
        <CardContent className="space-y-4">
          ...
          {data?.status === "ativo" && (
            <div className="flex items-start gap-3 rounded-lg border border-success/30 bg-success/5 p-4">
              ...
              <p className="text-sm">Sua assinatura está em dia. Obrigado por confiar no ClinicFlow!</p>
            </div>
          )}
          ...
          {!data?.asaasSubscriptionId && isAdmin && !data?.cnpj && (
            <div className="space-y-3">
              <div className="flex items-start gap-3 rounded-lg border border-warning/30 bg-warning/5 p-4">
                ...
                <p className="text-sm">Para gerar a cobrança, precisamos do CPF ou CNPJ da clínica (dados de cobrança exigidos pelo Asaas).</p>
              </div>
